...
2. **KPI Studio** (`/kpi`) - Advanced KPI visualization with 5 tabs: Overview (SQDCM summary cards, bar chart, radar), **Suivi Temporel** (full KPI time-tracking system — see below), Chart Studio (configurable chart grid supporting Bar/Line/Pie/Area/Stacked Bar; per-card config: title, categories, department, aggregation, color palette, legend, axis labels; export PNG/SVG/PDF/Print per chart; add/remove/switch type), KPI Data (filterable table with trend delta, CRUD), Report (full PDF/print report with SQDCM summary + embedded charts + KPI table). Uses html-to-image + jspdf for high-quality exports.
   - **Suivi Temporel** (`client/src/components/kpi-time-tracker.tsx`): KPI time-tracking system. Select any KPI, view by Jour/Semaine/Mois with synthetic time-series generated from trend data. Supports Line/Bar/Area chart types; date range picker; period comparison (any KPI vs any period); filters (department, shift, production line); overlays: moving average, target line, alert threshold markers, spike/anomaly highlighting (2-sigma detection); editable chart title/palette/y-axis label; stats sidebar (min/max/avg/spikes/above-target); export PNG/SVG/PDF/Print.
